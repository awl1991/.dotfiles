...
POWERLEVEL9K_CUSTOM_BUILT_WITH_BACKGROUND="16"
POWERLEVEL9K_CUSTOM_BUILT_WITH_FOREGROUND="16"
...
built_with() {
    ...
    for row in $(jq < "$AWL_ZSH"/built-with-config.json -r '.[] | @base64'); do
        _jq() {
...
built_with_check_file_exists() {
    ...
    if [ -f "$fileToCheck" ]; then
        if [ "$showIcon" = 'true' ]; then ROW_OUTPUT="$ROW_OUTPUT %F{23}$icon "; fi
    fi
}
